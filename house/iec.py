...
class IEC(object):
    ...
    def __init__(self, data, prediction_window=16 * 60):
        ...
        self.algorithms = {
            ...
            "GP Custom2": partial(self.gaussian_process_regression, training_window=1440*60, k=5, kernel="custom2", 
                recent_baseline_length=10),
            "rnn lstm": partial(self.rnn_lstm, training_window = 1440*60*5, k=7, recent_baseline_length=5),
            ...
            "ARM_rnn": partial(self.ARM_rnn, training_window= 1440*60, k=7, recent_baseline_length=5, 
                num_samples = 2000, nn_file = False, trained_nn_file = "")
        }
    ...
    def ARM_rnn(self, training_window= 1440*60, k=7, recent_baseline_length=5, 
                num_samples = 2000, nn_file = False, trained_nn_file = ""):
...
#        X["b_predictor"] = training_df["b_predictor"]
   ...
   #     Y = np.reshape(Y, (Y.shape[0], 1, Y.shape[1]))
        ...
        # Fit the network
        history = model.fit(train_X, train_y, epochs=1000, batch_size=72, validation_data=(val_X, val_y), verbose=1, shuffle=False, callbacks=callbacks_list)
        ...
        for i in range(yhat.shape[1]):
            df['pred_energy_v'+str(i)] = scaler.inverse_transform(concatenate(((yhat[:, i].reshape(yhat.shape[0],1))
                                                                               , test_X_New[:, 1:features]), axis=1))[:,0].tolist()
            df['act_energy_v'+str(i)] = scaler.inverse_transform(concatenate(((test_y[:, i].reshape(test_y.shape[0],1))
                                                                              , test_X_New[:, 1:features]), axis=1))[:,0].tolist()
        return df
...
#        X["b_predictor"] = training_df["b_predictor"]
   ...
   #     Y = np.reshape(Y, (Y.shape[0], 1, Y.shape[1]))
    ...
    ## new function to add -- what we get from this is the model. 
        ...
        # TODO: include time of day, not time of year or anything 
    ...
    def RNN_resids_no_lag(self, training_window=1440 * 60, k=7,recent_baseline_length=5, 
        ...
        training_data = pd.read_csv("resids_baseline.csv") # ? self.data
